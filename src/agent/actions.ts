...
export async function executeAction(
  ...
  switch (action.type) {
    case "login": {
      ...
      console.log(
        `[LOGIN] Username fields found: ${await usernameInput.count()}`,
      );

      console.log(
        `[LOGIN] Password fields found: ${await passwordInput.count()}`,
      );

      console.log(
        `[LOGIN] Login buttons found: ${await loginButton.count()}`,
      );
      ...
      const username = action.username ?? "testuser";
      const password = action.password ?? "testpass";

      await usernameInput.fill(username);
      await passwordInput.fill(password);

      console.log(
        "[ACTION] Login form filled using runtime credentials.",
      );
      ...
      console.log("[ACTION] Login completed.");
